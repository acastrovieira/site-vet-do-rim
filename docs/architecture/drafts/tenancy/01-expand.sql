...
-- Architecture review artifact for ADR-001 only.
-- Assumes the current migration chain through 20260626010000.
...
-- Fail early when this draft is pointed at a schema other than the reviewed one.
DO $$
DECLARE
  required_table text;
BEGIN
  IF current_setting('server_version_num')::integer < 150000 THEN
    RAISE EXCEPTION 'ADR-001 draft requires PostgreSQL 15+; remote version must match supabase/config.toml';
  END IF;
