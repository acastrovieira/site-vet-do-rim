...
async function listE2EUsers() {
  ...
  while (true) {
    ...
    for (const user of data.users) {
      if (matchesRun(user.email, 'e2e-vet-labcrud-')) {
...
async function collectData() {
  const tutorPattern = runId ? `Tutor E2E CRUD ${runId}%` : 'Tutor E2E CRUD labcrud-%'
  const petPattern = runId ? `Paciente E2E CRUD ${runId}%` : 'Paciente E2E CRUD labcrud-%'
